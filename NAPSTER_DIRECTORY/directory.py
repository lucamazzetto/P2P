...
# " len(listFile): ", len(listFile)
		while i < len(listFile):
			...
			while j < len(listFile[i].idsess):
				#se trova stesso id lo cancella (nota l’assenza di break! :) )
			...
			#se il numero di id è nullo (file con un solo peer) elimino il file
...
def addf(c_sock):
	...
	if (sessionLogin == 1):
		...
		for files in listFile:
			#se trovo stesso md5
			if (files.md5 == md5):
				print "File già presente in directory: aggiornamento nome file."
				filePresente = 1
				files.nome = nomef
				trovato = 0
				#ciclare gli idSess
	...
	if (sessionLogin == 1):
		...
		for files in listFile:
			...
			if files.md5 == md5:
				...
				#rimuovo direttamente l’idSessione e conto i file rimanenti
